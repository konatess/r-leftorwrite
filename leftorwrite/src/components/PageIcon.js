...
export default function PageIcon (props) {
    return <div className='row'>
        <a className={ "pageIcon" + (props.page.contentFinished ? "" : " unfinPageIcon")} href={strings.fRoutes.writep.replace(":pageid", 6)}>
        ...
        <span className='icon-title'>{props.page.title}</span>
    </div>
}